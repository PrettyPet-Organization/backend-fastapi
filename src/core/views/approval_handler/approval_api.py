...
@approval_router.get(
    "/application/retreive/user/{user_id}",
    response_model=list[ApplicationResponseTemplate],
)
async def retreive_users_applications(
    ...
    stmt = select(ProjectRoleResponseBase).where(
...
@approval_router.get(
    "/application/retreive/project/{project_id}",
    response_model=ProjectRolesResponsesTemplate,
)
async def retrieve_project_applictaions(
    ...
    stmt = (
        select(ProjectBase)
        .options(
            joinedload(ProjectBase.roles).joinedload(
                ProjectRolesBase.project_role_response
            )
        )
        .where(
    ...
    if not project_data:
        raise HTTPException(status_code=404, detail="No such project exists")
...
async def apply_for_a_position(
    ...
    stmt = select(ProjectRolesBase).where(ProjectRolesBase.id == role_id)
    ...
    if role_existance.number_of_needed <= 0:
        raise HTTPException(
            status_code=403, detail="The project has no spare role for you"
        )

    application = ProjectRoleResponseBase(project_role_id=role_id, user_id=user_data.id)

    db.add(application)
    await db.commit()
    await db.refresh(application)

    return Response(status_code=201, content="You have applied for the project ")


@approval_router.post(
    "/application/review/{application_id}", response_model=ProjectRolesResponsesTemplate
)
async def review_application(
    ...
    stmt = (
        select(ProjectRoleResponseBase)
        .options(
            joinedload(ProjectRoleResponseBase.project_role).joinedload(
                ProjectRolesBase.project
            )
        )
        .where(ProjectRoleResponseBase.id == application_id)
    )
    ...
    if not application:
        raise HTTPException(
            status_code=404, detail="There is no application going by such id"
        )

    if user_data.id != application.project_role.project.creator_id:
        raise HTTPException(
            status_code=403, detail="You are not allowed to approve people for roles"
        )
    ...
    match action.action:
        case "accept":
            stmt = (
                ...
                .where(ProjectRolesBase.id == application.project_role.id)
                .values(number_of_needed=application.project_role.number_of_needed - 1)
            )
            ...
            stmt = (
                update(ProjectRoleResponseBase)
                .where(ProjectRoleResponseBase.id == application_id)
                .values(
        ...
        case _:
            raise HTTPException(status_code=400, detail="Incorrect action")
...
@approval_router.delete(
    "/application/delete/{application_id}", response_model=ProjectRolesResponsesTemplate
)
async def resign_from_project(
    ...
    if not application:
        raise HTTPException(status_code=404, detail="There is no such application")
    if user_data.id not in [
        application.project_role.project.creator_id,
        application.user_id,
    ]:
        raise HTTPException(
            status_code=403,
    ...
    if application.application_status == "approved":
        stmt = delete(ProjectRoleResponseBase).where(
        ...
        stmt = (
            update(ProjectRolesBase)
            .values(number_of_needed=application.project_role.number_of_needed + 1)
            .where(ProjectRolesBase.id == application.project_role_id)
        )
