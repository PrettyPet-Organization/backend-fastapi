...
async def skill_data(
    ...
    stmt = (
        ...
        .options(
            joinedload(ProjectRolesBase.project), selectinload(ProjectRolesBase.skills)
        )
    ...
    if project_data.project_id != user_data.id:
        raise HTTPException(
            detail="You are not allowed to change this data", status_code=403
        )
    if skill_id in [i.id for i in project_data.skills]:
        raise HTTPException(
            status_code=409,
            detail="This connection was already present in the database",
        )
    ...
    new_role = ProjectRoleSkillsAssociation(role_id=role_id, skill_id=skill_id)
...
async def delete_skill_connection(
    ...
    if not role_data:
        raise HTTPException(
            status_code=404, detail="Either the project or the role was not found"
        )
    if role_data.project.creator_id != user_data.id:
        raise HTTPException(
            status_code=403, detail="You are not allowed to make such changes"
        )

    stmt = select(ProjectRoleSkillsAssociation).where(
        and_(
    ...
    if not is_existing:
        raise HTTPException(
            status_code=409, detail="There was no such connection in the first place"
        )

    stmt = delete(ProjectRoleSkillsAssociation).where(
        and_(
            ProjectRoleSkillsAssociation.role_id == role_id,
            ProjectRoleSkillsAssociation.skill_id == skill_id,
        )
