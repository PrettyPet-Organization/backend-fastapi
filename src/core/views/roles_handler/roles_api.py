...
@roles_router.post(
    ...
    status_code=201,
    response_model=RoleOutputTemplate,
)
async def add_role_to_the_project(
    ...
    stmt = select(RoleTypesBase).where(RoleTypesBase.id == role_id)
    ...
    if not role_in_the_database:
        raise HTTPException(
            status_code=404, detail="There was no such role type found in the database"
        )
    ...
    if not project_data_scalar:
        raise HTTPException(
            status_code=404, detail="There was no project with such id in the database"
        )
    if project_data_scalar.creator_id != user_data.id:
        raise HTTPException(
            status_code=403,
            detail="Only creator is allowed to add new roles to the project",
        )

    stmt = select(ProjectRolesBase).where(
        and_(
    ...
    if is_existing_connection:
        raise HTTPException(
            status_code=409, detail="This role already is required in the project"
        )

    new_role = ProjectRolesBase(
        role_type_id=role_id, project_id=project_id, **(new_role_data.model_dump())
    )
    db.add(new_role)
    await db.commit()
    ...
    stmt = (
        ...
        .options(joinedload(ProjectRolesBase.role_types))
        .where(
            ProjectRolesBase.role_type_id == role_id,
            ProjectRolesBase.project_id == project_id,
        )
...
@roles_router.get(
    ...
    status_code=200,
    response_model=list[RoleOutputTemplate],
)
async def get_project_roles(
    ...
    stmt = (
        select(ProjectRolesBase)
        .options(joinedload(ProjectRolesBase.role_types))
        .where(ProjectRolesBase.project_id == project_id)
    )
...
@roles_router.put(
    "/projects/{project_id}/roles/{role_id}",
    status_code=201,
    response_model=RoleExtendedOutputTemplate,
)
async def change_project_role(
    ...
    stmt = (
        select(ProjectRolesBase)
        .options(
            joinedload(ProjectRolesBase.project).joinedload(ProjectBase.creator),
    ...
    if not role_info_scalar:
        raise HTTPException(
            status_code=404, detail="There was no such role found in the project"
        )
    if role_info_scalar.project.creator_id != user_data.id:
        raise HTTPException(
            status_code=403, detail="You are not allowed to change data here"
        )
    if not role_id == role_info_scalar.id:
        raise HTTPException(
            status_code=404, detail="The role was not found in the project"
        )

    stmt = (
        update(ProjectRolesBase)
        .values(**(project_role_data.model_dump()))
        .where(
            and_(
                ProjectRolesBase.role_type_id == role_id,
                ProjectRolesBase.project_id == project_id,
...
async def delete_role(
    ...
    stmt = (
        select(ProjectBase)
        .options(joinedload(ProjectBase.creator))
    ...
    if not project_data:
        raise HTTPException(
            detail="There was no such project in the database", status_code=404
        )
    if project_data.creator_id != user_data.id:
        raise HTTPException(
            status_code=403,
            detail="You are not allowed to change data for someone else's project",
        )
    ...
    if not is_role_existing:
        raise HTTPException(
            status_code=404, detail="There was no such role in the database"
        )
    ...
    if not is_existing_connection:
        raise HTTPException(
            status_code=409,
            detail="There was already no such connetcion in the database",
        )

    stmt = delete(ProjectRolesBase).where(
        and_(
            ProjectRolesBase.project_id == project_id,
            ProjectRolesBase.role_type_id == role_id,
        )
